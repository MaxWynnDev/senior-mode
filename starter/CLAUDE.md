...
This file lives at `~/.claude/CLAUDE.md` and loads into every session in
every repo. Keep it short: every line costs context on every turn.
Project-specific facts (build commands, stack, deploy flow) go in the
...
(Windows) or anything else that kills every Node process. Claude Code
itself runs on Node; killing them all kills the session mid-operation.
...
The operating doctrine lives in `~/.claude/ENGINEERING-PRINCIPLES.md`.
...
`CLAUDE.md` for facts every session needs, a memory (`#` in the prompt)
for how the user works, a hook when it must be enforced mechanically.
