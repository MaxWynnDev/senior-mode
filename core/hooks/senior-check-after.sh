#!/usr/bin/env bash
# Stop hook (Claude Code hook contract; wired by every adapter that has a Stop event).
...
#
# Push-turn skip: if pre-push-checklist.sh set the SESSION-SCOPED push
...
# agent sessions on the same machine cannot clear each other's
# flags.)
#
